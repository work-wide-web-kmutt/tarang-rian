...
Load this reference when changing persisted state, storage adapters, hydration, or legacy-data migration.
...
Persist only JSON-serializable source state. Never persist the `actions` object, functions, or values that can be derived from persisted state.

## Versioning and migration

- Add `version` when the persisted shape changes.
- Implement `migrate(persistedState, version)` defensively for malformed and legacy values. Return a valid current state even when migration cannot recognize input.
...
- `academic-context.ts` restores a valid academic term or falls back to the latest catalog term.
...
- For SSR or asynchronous storage, explicitly consider hydration timing and whether delayed `rehydrate()` or a hydration guard is required. This Vite app currently uses browser-local persistence rather than an SSR store.
